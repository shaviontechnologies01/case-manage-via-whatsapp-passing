/**
 * Intelligent WhatsApp Message Parser & Classifier for AO Smith & Water Purifier Service Cases
 * 
 ...
 * 
 * Standard Sheet Routing:
 * - If source tag / start tag is 'Aarvee' / 'Arvee' -> Sheet 'Aarvee'
 * - Default -> Sheet 'Shubh'
 */

export const KNOWN_SOURCE_TAGS = ['Auto', 'Repit', 'Repeat', 'Aarvee', 'Arvee', 'Phone', 'Order'];

export const CANONICAL_CASE_TYPES = ['Installation', 'Complaint', 'Order', 'Auto'];
...
/**
 * Checks if a string is an Indian Pincode (e.g., 394270, 395007, 394210, etc.)
 */
...
/**
 * Classifies a case into exactly one of 4 canonical Master Excel Record Types:
 * - 'Auto'
 * - 'Complaint'
 * - 'Installation'
 * - 'Order'
...
export function classifyCaseRecordType({
  ...
  // Rule 1: Auto - Message starts with Auto or has Auto source tag
...
/**
 * Splits bulk concatenated text into individual message segments
 */
export function splitMessagesClient(rawBatchText) {
  ...
  // Pattern matching start of message: known source tag or 7-8 digit case number on a new line
  ...
  const validCount = segmentsByStart.filter(s => /\b\d{7,8}\b|[6-9]\d{4}[\s-]?\d{5}/.test(s)).length;

  if (segmentsByStart.length > 1 && validCount >= segmentsByStart.length * 0.5) {
...
/**
 * Parses a single WhatsApp message string into structured fields with exact field matching
 */
export function parseSingleMessageClient(messageText, defaultTechnician = '', customCaseTypes = DEFAULT_CASE_TYPES) {
  ...
  // Step 1: Extract Source tag (Auto, Repit, Repeat, Aarvee, Arvee, Phone, Order)
  let sourceTag = '';
  ...
  if (prefixCaseMatch) {
    ...
    if (caseNumMatch && !isPincode(caseNumMatch[0])) {
      ...
      warnings.push('No Case #');
    }
  }

  // Step 3: Extract Phone numbers (10 digits starting with 6, 7, 8, or 9)
  ...
  // Check for trailing date + service amount like: --28/06/2025--400/- or --25/12/2023--400/-
  ...
  // Check trailing notes (e.g. Out warranty, single vali, double vali, cancle done, cancelled, etc.)
  ...
  // Check for trailing prices (e.g., "250 400", "2900 400", "250+400", "3000 400(single vali)", "400/-", "Rs. 400")
  ...
  if (trailingPricesDualMatch) {
    ...
    // Single trailing amount (e.g. 400/- or 2900)
  ...
  // Step 5: Extract Model / Device Category inside [...] or (...) brackets
  let rawDetectedType = '';
  let product = '';
  let customerName = '';
  let beforeBracketStr = '';

  const allKnownCaseTypes = Array.from(new Set([...customCaseTypes, ...DEFAULT_CASE_TYPES]));
  ...
  if (bracketMatch) {
    ...
    // Clean any phones from beforeBracketStr so phone numbers never pollute customerName
    ...
    if (!matchedCaseType) {
      // Try finding case type keyword anywhere in cleanBefore
      for (const ct of allKnownCaseTypes) {
    ...
    // Note: Any text after brackets (address, location, etc.) is skipped completely as requested.

  } else {
    // No brackets found - check for standalone model codes like GCUR303, Revito, Copper, Titan, Classic, etc.
    ...
    // Remove phone matches from working text
    ...
    // Check if there was a newline separating name from address/other text
    ...
    if (!customerName) {
      // Find where address-like patterns start and extract only the customer name before it
    ...
    // Detect Case Type from customerName if present
  ...
  // Step 6: Customer Name Finalization and Cleansing
  // Strip any remaining phone digits or stray symbols from customerName
  ...
  // If customer name still empty, provide clean placeholder
  if (!customerName) {
    customerName = caseNumber ? `Customer (${caseNumber})` : 'Customer';
  }

  // Address is intentionally skipped and kept blank as per user instruction
  const address = '';
